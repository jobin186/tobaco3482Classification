...
imdb.expDir = ['/home/jobinkv/subFigClas/exp01/',imdb.feature]; % 9--> 1:8k,10-->1:end
opts.useGpu = true;
tsne = true;
%====================================================================================
			%%%%%% Internal setups  %%%%%%%%%%
...
imdb.resultPath = fullfile(imdb.expDir, sprintf('data.mat')) ;
...
if exist(imdb.resultPath)
	...
	disp('calculating chi-squre distance');
    data.similarityMatrix=pdist2(encode',encode','euclidean'); %euclidean chisq
    %tmax = max(max(data.similarityMatrix));
    %data.similarityMatrix = data.similarityMatrix./tmax; 
    data.figNames=imdb.images;
    save(imdb.resultPath,'data');
end
end
