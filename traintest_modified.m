...
train = ismember(imdb.segments.set, 2) ;
test = ismember(imdb.segments.set, 1) ;
info.classes = ones(1,size(imdb.segments.label,1)) ;
C = 1 ;
w = {} ;
b = {} ;
...
for c=1:numel(info.classes)
  ...
  np = sum(y(train) > 0) ;
 testP = sum(y(test)>0)
disp('No of positive samples in class = ')
c
testP
  nn = sum(y(train) < 0) ;
  n = np + nn ;

  [w{c},b{c}] = vl_svmtrain(psi(:,train & y ~= 0), y(train & y ~= 0), 2/(n* C), ...
...
fprintf('mAP train: %.1f, test: %.1f\n', ...
  mean(info.train.ap)*100,mean(info.test.ap)*100);%, ...
%keyboard
%save ('SvmWeightsV2.mat','info');
 % mean(info.test.ap)*100);

%figure(1) ; clf ;
%subplot(3,2,1) ;
%bar([info.train.ap; info.train.ap]')
%xlabel('class') ;
%ylabel('AP') ;
%legend(...
