...
imdb.expDir = ['/home/jobinkv/rvlV1/exp03/',imdb.feature]; % exp02--> 16 gmm, exp03---> 32 gmm
opts.useGpu = true;
...
if isempty(imdb.encoder)
    ...
    for i =1:imdb.batchSize:20000 %numel(imdb.Sets{1}.imagePath)
            tic
...
%====================================================================================
cnt=1;
for i = 1:imdb.batchSize:numel(imdb.Sets{1}.imagePath)/2
        tic
        out{cnt} = extractFeature(imdb,i,'mode','train');
	cnt=cnt+1;
        toc
end
for i = 1:imdb.batchSize:numel(imdb.Sets{2}.imagePath)/2
        tic
        out{cnt} = extractFeature(imdb,i,'mode','test');
	cnt=cnt+1;
        toc
end
for i = 1:imdb.batchSize:numel(imdb.Sets{3}.imagePath)/2
        tic
...
disp(['No of testing images = ',int2str(cnt*imdb.batchSize)])
end
...
fclose(fileID);
!scp *cnn.txt jobinkv@10.2.16.195:/var/www/cgi-bin/dv2/
%------------------------------------------------
...
%!./copyTrainTestSet.sh
opts = vl_argparse(opts, varargin) ;
imdb.imageDir = fullfile(imdb.dataLoc, 'labels') ;
trainFileId = fopen([imdb.imageDir,'/train.txt']);
...
for s = 1:length(imageSets),
    ...
    if numberOfImages>=1
		...
		tempLoc = C{1,1}(j);
                imdb.Sets{s}.imagePath{j} = [imdb.dataLoc,'images/',tempLoc{1}];
		imdb.images{j}=tempLoc{1};
                imdb.Sets{s}.label(j) = str2num(C{2}{j});
          end
    end
    fclose(fileID);
