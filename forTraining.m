...
imdb.expDir = ['/home/jobinkv/similarityMatrix/newsetv4/',imdb.feature]; % 9--> 1:8k,10-->1:end
opts.useGpu = true;
...
%%%%%% Creator encoder %%%%%%%%%%
%====================================================================================
imdb.batchSize =numel(imdb.images);% 1000; %numel(imdb.images);
switch mode
    case 'train'
        cnt=1;
        if isempty(imdb.encoder)
            disp('no encoder found!');
            for i =1:1000:3000 %numel(imdb.images) %numel(imdb.Sets{1}.imagePath)
                tic
        ...
        %       toc
        %end
        %disp(['No of training images = ',int2str(cnt*imdb.batchSize)])
    ...
    case 'test'
        ...
        if exist(imdb.resultPath)
            ...
            disp('please train first')
        end
        for i = 1:imdb.batchSize:numel(imdb.Sets{1}.imagePath)
            tic
            out = extractFeatureTest(imdb,i,'mode','train');
	    k=1;
            for j=1:numel(out.feature)
            	encode{k}=out.feature{j};
            	k=k+1;
            end
    	    encode=cat(2,encode{:});
            scores = test_modified(encode,info);
	    clear encode;
	    writeAnnotationData(imdb,i,'mode','train','score',scores)
            toc
        end
...
disp('yahooooo')
%--------------------------------------------------
function writeAnnotationData(imdb,ii,varargin)
opts.mode='test';
opts.score=[];
opts = vl_argparse(opts, varargin) ;
fileID = fopen('machineannotationV3.txt','a');
inx=1;
for i=ii:ii+imdb.batchSize-1
	kk=strsplit(imdb.Sets{1}.imagePath{i},'/');
    fprintf(fileID,'%15s, %f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n',...
	kk{6},opts.score{1}(inx),opts.score{2}(inx),opts.score{3}(inx),opts.score{4}(inx),...
	opts.score{5}(inx),opts.score{6}(inx),opts.score{7}(inx),opts.score{8}(inx),opts.score{9}(inx),opts.score{10}(inx),...
   	opts.score{11}(inx),opts.score{12}(inx),opts.score{13}(inx),opts.score{14}(inx),opts.score{15}(inx),...
   	opts.score{16}(inx),opts.score{17}(inx),opts.score{18}(inx),opts.score{19}(inx),opts.score{20}(inx));
	inx=inx+1;
end
%--------------------------------------------------
...
for i=ii:ii+imdb.batchSize-1
    ...
    fprintf(fileID,'%d,%d, %15s, %d\n',i,id, imdb.Sets{id}.imagePath{i},label_id);
...
out.labeID = labeID;
%--------------------------------------------
function [out] = extractFeatureTest(imdb,ii,varargin)
opts.mode='train';
opts = vl_argparse(opts, varargin) ;
no_labels = length(imdb.classes.name);
id=10;
...
labeID =[];
j=1;
fileID = fopen('Check2.txt','a');
for i=ii:ii+imdb.batchSize-1
    fprintf(fileID,'%15s\n',imdb.Sets{id}.imagePath{i});
    bachGround_patch{j} = imread(imdb.Sets{id}.imagePath{i});
    j=j+1;
end
fclose(fileID);
...
for s = 1:length(imageSets),
    ...
    if numberOfImages>=1
	    ...
	    temps =strfind(imdb.classes.name,C{2}{j});
            imdb.Sets{s}.label(j) = find(not(cellfun('isempty', temps)));
            %imdb.Sets{s}.label(j) =maplabel(C{2}{j},imdb.classes.name);
