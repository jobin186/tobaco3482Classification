function trainAnnotv1(varargin)
...
%%%%%% Creator encoder %%%%%%%%%%
%====================================================================================
imdb.batchSize =1000; %numel(imdb.images);% 1000; %numel(imdb.images);
switch mode
    case 'train'
        cnt=1;
        if isempty(imdb.encoder)
            disp('no encoder found!');
            for i =1:imdb.batchSize:numel(imdb.images) %numel(imdb.Sets{1}.imagePath)
                tic
...
fileID = fopen('machineannotation-v4.txt','a');
inx=1;
for i=ii:ii+imdb.batchSize-1
	...
	opts.score{5}(inx),opts.score{6}(inx),opts.score{7}(inx),opts.score{8}(inx),opts.score{9}(inx),...
	opts.score{10}(inx),...
